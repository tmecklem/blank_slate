...
- **Run tests frequently**: Run `mix test path/to/file_test.exs` (or the relevant subset) after every change, both test and implementation.
...
- **Test data**: Build test data through the fixture modules in `test/support/fixtures/` (generated alongside contexts by `mix phx.gen.*`). Add a fixture function there rather than inserting records inline in a test.
- **LiveView tests**: Follow the LiveView test guidelines in `AGENTS.md` — assert on element IDs with `has_element?/2` and `element/2`, never on raw HTML.

## When Working on a Feature

1. Start by understanding the requirement
2. Break it into small, testable behaviors
3. List the behaviors as individual TDD cycles in your todo list
4. Work through each cycle one test at a time: Red -> Green -> Refactor
5. After all cycles, run `mix precommit` (compile with warnings as errors, format, full test suite)
